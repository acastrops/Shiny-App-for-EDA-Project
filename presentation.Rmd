...
# Loading in data
collegedata <- read.csv("C:\\DS\\Data Munging\\GroupProject1\\ShinyProject\\MungingProjectsDataofInterest.csv")
...
map_year <- function(specific_year){
dates <- unique(collegedata$Year)
state_data <- collegedata %>%
    filter(Year == dates[specific_year]) %>% 
    group_by(STABBR) %>% 
...
manipulate(map_year(specific_year), specific_year = slider(1, 14, step=1, initial = 1))
```

![map](map.png)
```{r}
f3 <- function(n){
  df <- data.frame(x = numeric(n), y = character(n), z = character(n), stringsAsFactors = FALSE)
  for(i in 1:n){
    df$x[i] <- i
    df$y[i] <- toString(i)
    df$z[i] <- i**2
